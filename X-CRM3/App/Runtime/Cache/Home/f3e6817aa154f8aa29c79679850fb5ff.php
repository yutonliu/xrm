<?php if (!defined('THINK_PATH')) exit();?><div class="bjui-pageContent tableContent">
    <form action="/index.php?s=/Home/Shou/add/navTabId/<?php echo CONTROLLER_NAME;?>" class="pageForm" data-toggle="validate">
		<input type="hidden" name="id" value="<?php echo ($id); ?>">
        <div class="pageFormContent" data-layout-h="0">
          <table class="table table-condensed table-hover" width="100%">
           <tbody>
			<tr><td><label for='mingcheng_input' class='control-label x85'>合同名称:</label><input type='text' id='mingcheng' name='mingcheng'  size='20' data-toggle='lookup' data-url='/index.php?s=/Home/basic/hetong/navTabId/<?php echo CONTROLLER_NAME;?>'  value='<?php echo ($Rs["mingcheng"]); ?>'  ></td></tr>
<tr><td><label for='type_input' class='control-label x85'>收款类型:</label><input type='text' id='type' name='type'  size='20' data-toggle='lookup' data-url='/index.php?s=/Home/basic/sktype/navTabId/<?php echo CONTROLLER_NAME;?>'  value='<?php echo ($Rs["type"]); ?>'  ></td><td><label for='jine_input' class='control-label x85'>收款金额:</label><input type='text' id='jine' name='jine' data-rule='required;number' size='20'   value='<?php echo ($Rs["jine"]); ?>'  ></td></tr>
