...
if(isset($_REQUEST['jingban']) && $_REQUEST['jingban'] != ''){$map['jingban'] =array('EQ', urldecode(I('jingban'))); }
 ...
 $this->assign('typelist', $typelist); $shenhelist =  M($this->dbname)->where(array('shenhe'=>array('neq','')))->distinct('shenhe')->field('shenhe')->select();
 $this->assign('shenhelist', $shenhelist); 
 $jingbanlist =  M($this->dbname)->where(array('jingban'=>array('neq','')))->distinct('jingban')->field('jingban')->select();
  ...
  public function _befor_update($data){
   ...
   public function _befor_view($id){
    ...
    public function _befor_del($id){
     $hid=M('fukuan')->where('id='.$id)->getField('hid');
	 $jine=M('fukuan')->where('id='.$id)->getField('jine');
	 M("hetong")->where('id='.$hid)->setDec('yifu',$jine);
   ...
   public function outxls() {
		...
		$list = $model->where($map)->field('id,bianhao,hid,mingcheng,title,leixing,type,jine,jingban,beizhu,shenheren,shenhe')->select();
	    $headArr=array('ID','合同编号','合同ID','合同名称','客户名称','付款类型','付款方式','付款金额','经办人','备注','审核人','审核状态');
	    $filename='付款记录';
	...
	public function shenhe(){
	 if(IS_POST){
	   ...
	   D($this->dbname)->save($upda);
	    ...
	    M("shenhe")->add($data);
		if($_POST['title']=="审核通过"){
		   $hid=M('fukuan')->where('id='.I("xid"))->getField('hid');
	       $jine=M('fukuan')->where('id='.I("xid"))->getField('jine');
	       M("hetong")->where('id='.$hid)->setInc('yifu',$jine);
